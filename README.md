...
- 📷 **二维码工具**：支持二维码的实时配置生成、下载与复制，以及对图片文件、拖拽图像或剪贴板截图粘贴（Ctrl+V）的高精度解码。
- 🔑 **密码与密钥生成器**：生成强随机高熵密码和开发密钥 Token，支持多字符集勾选，支持一键排除易混淆字符（如 o、O、0、l、1）及密码批量生成。
- ⏰ **Cron 表达式助手**：Cron 表达式的中文解析翻译、运行时间预测，以及支持基于图形化勾选自动拼接 Cron 的可视化生成器。
